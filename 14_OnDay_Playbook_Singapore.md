# 14 — On-the-Day Playbook: Singapore (TBD)

**Status:** Placeholder — cannot be finalized until the Singapore venue and local partner are confirmed (milestone M3, target Week 4; see [01_TI_MedTech_Partnership_Clarification](01_TI_MedTech_Partnership_Clarification.md) Q4 and [11_Risk_Register](11_Risk_Register.md) risk #9).
...
- Whether the Melbourne workshop (Tita & Dr Irwan, Day 1) streams from the Singapore venue, TI MedTech's own office, or remote — determines whether Singapore's AV setup needs outbound streaming capability, or this is entirely a Melbourne-side concern

## Action Required

1. Resolve [01_TI_MedTech_Partnership_Clarification](01_TI_MedTech_Partnership_Clarification.md) Q4 — does TI MedTech already have a Singapore venue/partner, or does Leon need to make an introduction?
2. Once a partner is identified, rebuild this document using the Melbourne playbook as the template, with that partner's staff filling the Venue Lead / Logistics Lead / Tech Lead roles.
3. Update [05_Budget_Revenue](05_Budget_Revenue.md) Singapore section from placeholder figures to real quotes once the venue is known.
4. Update [04_Schedule.json](04_Schedule.json) → `onTheDayCriticalPath.singapore` with real times once the run-of-show is built.

**Do not treat Melbourne's plan as a fallback for Singapore** — if this isn't resolved by Week 4, escalate to a reduced-scope or virtual-participation Singapore format rather than letting it drift unplanned into event week (see [11_Risk_Register](11_Risk_Register.md) risk #9 mitigation).
